...
class GoalInfoViewmodel extends ChangeNotifier {
  ...
  Future<void> init() async {
    ...
    _goalProgress = _goal.getGoalProgress(_costItemRepo.costItems);
    _currentGoalProgress = _goal.getCurrentGoalProgress(_costItemRepo.costItems, DateTime.now());
    _isInit = true;
  ...
  DateTime get chartStartDate {
    if (_goal.goalTracking == GoalTrackingPeriod.monthly) {
      ...
      // if
      if (DateTime.now().standard.addDay(-49).isAfter(_goal.startDate!)) {
        return DateTime.now().standard.addDay(-49);
      } else {
  ...
  int get targetReachingDays =>
      dailyData.entries
          .where((entry) => (entry.value?.expense ?? 0) < (_goal.target! / dayinCurrentMonth))
          .length;
  ...
  Map<String, String> get goalInfo => {
    ...
    "Categories":
        _goal.categories?.map((el) => _categoryRepo.getCategoryById(el).name).join(",") ?? "NA",
