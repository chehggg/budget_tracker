...
class ColorSettingsTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return CustomSettingsTile(
      trailingWidget: Container(height: 30,  width: 30, decoration: BoxDecoration(borderRadius: BorderRadius.circular(100), color: selectedColor),), 
      title: "Color", 
      onTap: () => showDialog(
        context: context, 
        builder:(context) {
              ...
              return AlertDialog(
                ...
                content:Column(
                  ...
                  children: [
                    Text("Spacing between list tiles."),
                    SizedBox(height: 8,),
                    Text("Higher value indicate more space between cost items and settings tile."),
                    // SizedBox(height: 40,),
                    Padding(
                      ...
                      child: SliderTheme(
                        ...
                        child: Slider(
                          ...
                          onChanged: (value) {
                            ...
                            context.read<ThemeModel>().updateSpacingValue(value.round());
                          }
                        ),
                      ),
                    )
              ...
              return AlertDialog(
                ...
                content: RadioGroup(
                  ...
                  child: Column(
                    ...
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text("Amount of item within a row in the cost entry screen."),
                      SizedBox(height: 20,),
                      ListTile(title: Text('3'), leading: Radio(value: 3), visualDensity: VisualDensity(vertical: -2, horizontal: -2), dense: true, contentPadding: EdgeInsets.all(0),),
                      ListTile(title: Text('4'), leading: Radio(value: 4), visualDensity: VisualDensity(vertical: -2, horizontal: -2), dense: true, contentPadding: EdgeInsets.all(0),),
                      ListTile(title: Text('5'), leading: Radio(value: 5), visualDensity: VisualDensity(vertical: -2, horizontal: -2), dense: true, contentPadding: EdgeInsets.all(0),),
                      SizedBox(height: 10,),
                    ],
                  ),
                ),
            ...
            builder: (context, setState) {
              return AlertDialog(
                ...
                content:Column(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text("Apply a scale factor to all font size. Higher value indicates larger font."),
                    Padding(
                      padding: const EdgeInsets.only(top: 60.0, bottom: 15),
                      child: SliderTheme(
                        data: SliderThemeData(
                        ...
                        child: Slider(
                          ...
                          onChanged: (value) {
                            setState(() => sliderValue = value);
                            context.read<ThemeModel>().updateFontSizeFactor(value.round());
                            setState((){});
                          }
                        ),
                      ),
                ...
                content:Column(
                  ...
                  children: [
                    Text(
                      ...
                      style: Theme.of(context).textTheme.bodyMedium!.copyWith(fontSize: 12, color: Theme.of(context).colorScheme.secondary),
                    ),
                    SizedBox(height:20),
                    DialogListTile(
                      title: "All", 
                      trailing: Switch(
                        ...
                        onChanged: (value) {
                          themeStateRead.updateDefaultBlur(ValueBlur.all, value);
                          setState(() => curDefBlur = themeStateRead.defaultBlur);
                        }
                      ), 
                    ),
                    Divider(),
                    DialogListTile(
                      title: "Summary", 
                      trailing: Switch(
                        value: [ValueBlur.all, ValueBlur.summary].contains(curDefBlur), 
                        onChanged: (value) {
                          context.read<ThemeModel>().updateDefaultBlur(ValueBlur.summary, value);
                          setState(() => curDefBlur = themeStateRead.defaultBlur);
                        }
                      ), 
                    ),
                    DialogListTile(
                      title: "List", 
                      trailing: Switch(
                        value: [ValueBlur.all, ValueBlur.list].contains(curDefBlur), 
                        onChanged: (value) {
                          context.read<ThemeModel>().updateDefaultBlur(ValueBlur.list, value);
                          setState(() => curDefBlur = themeStateRead.defaultBlur);
                        }
                      ),
                    ),
...
class SettingsSectionTitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      dense: true,
      title: Text(titleName, style: Theme.of(context).textTheme.titleLarge!.copyWith(color: Theme.of(context).colorScheme.primary),),
    );
  }
}
...
class CustomSettingsTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // final title = ti 
    return ListTile(
      title: title != null? Text(title!, style: Theme.of(context).textTheme.titleMedium) : 
      titleWidget,
      subtitle: subtitle,
      dense: true,
      ...
      builder:(context) {
        return AlertDialog(
          ...
          content: Padding(
            ...
            child: const Text("Warning: Loading a .csv file will overwrite all existing data. It is recommended to save the current data as backup."),
          ),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context), 
            ...
            TextButton(
              ...
              child: Text("Proceed", style: TextStyle(color: Colors.yellow),)
            ),
          ],
        );
      },
      ...
      onTap: () => showDialog(
        ...
        builder:(context) {
          return AlertDialog(
            ...
            content: const Text("Warning: All existing data will be removed. This action cannot be undone. It is recommended to keep a export current data as backup."),
            actions: [
              TextButton(
                onPressed: () => Navigator.pop(context), 
